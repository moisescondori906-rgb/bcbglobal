...
async function request(url, options = {}, retries = 2) {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      // Timeout más corto para auth (25s) vs otros endpoints (35s)
      const timeoutMs = isAuth ? 25000 : 35000;
      const timeoutId = setTimeout(() => controller.abort(), timeoutMs);

      // Verificación de estado de red antes de disparar
      ...
      const res = await fetch(finalUrl, { 
        ...
        if (err.name === 'AbortError' || err.message?.includes('aborted')) {
          const abortError = new Error('Conexión interrumpida o tiempo de espera agotado.');
          abortError.name = 'AbortError';
          throw abortError;
        }
        
        // Manejar ERR_NETWORK_IO_SUSPENDED y similares
        if (err.message?.includes('Failed to fetch') || err.message?.includes('NetworkError')) {
           const netError = new Error('Conexión de red suspendida o inestable.');
           netError.name = 'NetworkSuspendedError';
           throw netError;
        }
...
export const api = {
  ...
  tasks: {
    ...
    responder: (id, data = {}) => request(`/tasks/${id}/responder`, { method: 'POST', body: JSON.stringify(data) }),
  },
  levels: {
    list: () => request('/levels'),
  ...
  admin: {
    ...
    crearPremioRuleta: (data) => {
      const isFormData = data instanceof FormData;
      return request('/admin/premios-ruleta', { 
        method: 'POST', 
        body: isFormData ? data : JSON.stringify(data) 
      });
    },
    actualizarPremioRuleta: (id, data) => {
      const isFormData = data instanceof FormData;
      return request(`/admin/premios-ruleta/${id}`, { 
        method: 'PUT', 
        body: isFormData ? data : JSON.stringify(data) 
      });
    },
    eliminarPremioRuleta: (id) => request(`/admin/premios-ruleta/${id}`, { method: 'DELETE' }),
    ruletaForzada: () => request('/admin/ruleta/forzada'),
    crearRuletaForzada: (data) => request('/admin/ruleta/forzada', { method: 'POST', body: JSON.stringify(data) }),
    eliminarRuletaForzada: (id) => request(`/admin/ruleta/forzada/${id}`, { method: 'DELETE' }),
    logsRuleta: () => request('/admin/ruleta/logs'),
