        ...
        <div className="p-6 flex flex-col items-center justify-center min-h-[70vh] text-center space-y-8 animate-fade">
          <Card variant="premium" className="w-full flex flex-col items-center p-10 space-y-6">
            ...
            <Link to="/vip" className="w-full">
              <Button icon={Zap}>Subir de Nivel Ahora</Button>
            </Link>
          </Card>
        </div>
      </Layout>
    ...
    <Layout>
      ...
      <main className="px-5 space-y-6 pb-10 pt-4 animate-fade">
          ...
          <div className="relative z-10 space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-xl font-black text-white uppercase tracking-tight">Gana Comisiones</h3>
              </div>
            </div>
            
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-sav-surface border border-sav-border rounded-2xl px-5 py-3.5 flex items-center justify-between shadow-inner">
              ...
              <Link to="/invitar">
                <Button className="w-14 h-14 rounded-2xl" icon={ChevronRight} />
              </Link>
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card variant="flat" className="p-5 space-y-1">
            ...
            <p className="text-xl font-black text-white tracking-tight">{(resumen.ingresos_totales || 0).toFixed(2)}</p>
          </Card>
          <Card variant="flat" className="p-5 space-y-1">
            <div className="flex items-center justify-between">
            ...
            <p className="text-xl font-black text-white tracking-tight">{resumen.total_miembros || 0}</p>
          </Card>
        </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-1 mb-2">
            ...
            <h2 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">Estructura de Red</h2>
          </div>
          ...
          <div className="space-y-3">
            {niveles.map((n, i) => (
              <Card 
                key={n.nivel} 
                variant="outline" 
                className="p-5 flex items-center gap-4 group hover:border-sav-primary/20 transition-all"
                delay={i * 0.05}
              >
                <div className="w-12 h-12 rounded-2xl bg-sav-surface border border-sav-border flex items-center justify-center text-xs font-black text-white shadow-lg group-hover:bg-sav-primary/10 transition-colors">
                  {n.nivel}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-2">
                    <h4 className="text-[11px] font-black text-white uppercase tracking-widest">Nivel {n.nivel}</h4>
                    <Badge variant="success" className="px-2 py-0.5">+{n.porcentaje}%</Badge>
                  </div>
                  <div className="flex justify-between text-[9px] font-bold uppercase tracking-widest">
                    <span className="text-sav-muted">Activos: <span className="text-white">{n.total_miembros}</span></span>
        ...
        {/* Info Card */}
        <Card variant="flat" className="p-6 border-sav-primary/10 relative overflow-hidden">
          <div className="flex gap-4">
            <div className="w-10 h-10 rounded-xl bg-sav-primary/10 flex items-center justify-center text-sav-primary shrink-0">
              <Info size={20} />
            </div>
            <div className="space-y-2">
              <p className="text-[11px] font-black text-white uppercase tracking-widest">Sistema de Comisiones</p>
              <p className="text-[10px] text-sav-muted font-bold leading-relaxed uppercase tracking-widest">
                Cada tarea completada por tus invitados genera beneficios automáticos. <span className="text-white underline decoration-sav-primary underline-offset-4">¡Tu equipo es tu activo más valioso!</span>
              </p>
            </div>
          </div>
        </Card>
