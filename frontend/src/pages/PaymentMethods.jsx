...
export default function PaymentMethods() {
  ...
  const handleFile = async (e) => {
    ...
    try {
      // Just use the original file without heavy compression
    ...
    <Layout>
      <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-slate-50 pb-32">
        ...
        <main className="p-6 space-y-8 animate-fade">
          {/* Resumen del Pedido */}
          <section>
            <Card className="p-6 bg-white border-slate-200 shadow-xl relative overflow-hidden">
              <div className="flex justify-between items-center relative z-10">
                <div>
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Membresía Seleccionada</p>
                  <h3 className="text-xl font-black text-slate-900 uppercase">{selectedLevel?.nombre}</h3>
                </div>
                <div className="text-right">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Total a Pagar</p>
                  <p className="text-2xl font-black text-gray-900 tracking-tighter">
                    {Number(selectedLevel?.deposito || selectedLevel?.costo).toLocaleString('es-BO')} <span className="text-xs">Bs</span>
                  </p>
                </div>
              </div>
            </Card>
                ...
                filteredMetodos.map((m) => (
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className={cn("text-sm font-black uppercase", selectedMethod?.id === m.id ? "text-white" : "text-slate-900")}>
                        {m.nombre_banco || 'Pago QR'}
                      </p>
                      <p className={cn("text-[9px] font-bold uppercase tracking-widest", selectedMethod?.id === m.id ? "text-white/60" : "text-slate-500")}>
                        {m.nombre_titular}
                      </p>
                    </div>
                  </button>
          ...
          <AnimatePresence>
            {selectedMethod && (
              ...
              >
                ...
                <Card className="p-8 space-y-8 bg-white border-slate-200 rounded-[2.5rem] shadow-xl">
                  {/* QR Display */}
                  <div className="flex flex-col items-center gap-6">
                    <div className="p-4 bg-white rounded-[2rem] border border-slate-100 shadow-2xl">
                      <img 
                        src={selectedMethod.imagen_base64 || api.getMediaUrl(selectedMethod.imagen_qr_url)} 
                        alt="QR Pago" 
                        className="w-48 h-48 object-contain"
                      />
                    </div>
                    <Badge variant="outline" className="uppercase tracking-widest text-[9px] px-4 py-1.5 border-slate-200 text-slate-600 font-bold">Titular: {selectedMethod.nombre_titular}</Badge>
                  </div>
                  ...
                  >
                    {isOptimizing ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] font-black uppercase text-slate-500">Subir Comprobante</span>
                      </>
                    )}
                  </button>
