...
export default function AdminRankingV2() {
  ...
  return (
    <div className="space-y-10 animate-in">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-black text-white tracking-tighter uppercase italic">Ranking de Red</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="relative group flex-1 min-w-[200px] sm:min-w-[300px]">
            ...
            <input 
              ...
              className="admin-input pl-12 !h-14 focus:border-amber-500/30"
            />
          </div>
          <button 
      ...
      {/* Main Content: Table */}
      <div className="admin-table-container relative overflow-hidden">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="admin-table">
            <thead>
              <tr className="bg-white/[0.02]">
                <th className="px-8 py-6">Posición</th>
                <th className="px-8 py-6">Identidad</th>
                <th className="px-8 py-6 text-center">Inversión Red (ABC)</th>
                <th className="px-8 py-6 text-center">Liderazgo Institucional</th>
                <th className="px-8 py-6 text-right">Gestión de Rango</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                return (
                  <AnimatePresence key={u.id} mode="popLayout">
                    ...
                    >
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center font-black text-amber-500 border border-white/5 shadow-inner">
                            {u.nombre_usuario?.charAt(0).toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-black text-white uppercase tracking-tighter italic truncate w-32 sm:w-48">{u.nombre_usuario}</p>
                      ...
                      <td className="px-8 py-5 text-center">
                        ...
                        >
                          <div className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-500 font-black text-sm group-hover/btn:bg-amber-500 group-hover/btn:text-white transition-all shadow-lg shadow-amber-500/5">
                            <Users size={16} />
                            {u.invitados_count}
                            {isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                          </div>
                          <p className="text-[8px] font-bold text-slate-600 uppercase tracking-widest">Ver Estructura</p>
                        </button>
                      </td>
                      <td className="px-8 py-5 text-center">
                          ...
                          {updating === u.id ? (
                            ...
                            <>
                              <button 
                                ...
                                className={`p-3 rounded-xl transition-all border ${u.tipo_lider === 'lider_premium' ? 'bg-amber-500 text-white border-amber-400 shadow-xl shadow-amber-500/20' : 'bg-white/5 text-slate-500 border-white/5 hover:bg-amber-500/20 hover:text-amber-500 hover:border-amber-500/20'}`}
                                title="Líder Premium"
                              >
                                <Crown size={18} />
                              </button>
                              <button 
                                ...
                                className={`p-3 rounded-xl transition-all border ${u.tipo_lider === 'lider' ? 'bg-blue-600 text-white border-blue-500 shadow-xl shadow-blue-500/20' : 'bg-white/5 text-slate-500 border-white/5 hover:bg-blue-500/20 hover:text-blue-500 hover:border-blue-500/20'}`}
                                title="Líder Base"
                              >
                                <Star size={18} />
                              </button>
                              <button 
                                ...
                                className={`p-3 rounded-xl transition-all border ${!u.tipo_lider ? 'opacity-20 grayscale' : 'bg-white/5 text-slate-600 border-white/5 hover:bg-rose-500/20 hover:text-rose-500 hover:border-rose-500/20'}`}
                                title="Degradar"
                              >
                                <UserMinus size={18} />
                              </button>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                {['A', 'B', 'C'].map(level => (
                                  <div key={level} className="bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner text-center group hover:border-amber-500/20 transition-all">
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Nivel {level}</p>
                            ...
                            <div className="lg:col-span-2 bg-black/40 rounded-2xl p-6 border border-white/5 flex items-center justify-between relative overflow-hidden group">
        ...
        <div className="p-8 bg-amber-500/5 border-t border-white/5 flex items-center gap-6">
          <div className="p-4 rounded-2xl bg-amber-500/10 text-amber-500 shadow-inner border border-amber-500/20">
            <Award size={32} />
          </div>
