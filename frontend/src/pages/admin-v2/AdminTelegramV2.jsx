      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-black text-white tracking-tighter uppercase italic">Centro de Comandos</h1>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] flex items-center gap-2">
                <ShieldCheck size={14} className="text-blue-500" /> Control de flujos operativos y bots BCB Global
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <motion.div 
            ...
            className="admin-card shadow-2xl overflow-hidden border-white/5"
          >
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-blue-500/10 text-blue-500 border border-blue-500/20 shadow-inner">
                  <Shield size={20} />
                </div>
                <div>
              ...
              <button 
                ...
                className="admin-button-primary !h-12 !px-6 flex items-center gap-2 !bg-blue-600 hover:!bg-blue-500 shadow-blue-600/20"
              >
              ...
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    <th className="px-8 py-5">Chat ID</th>
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {Array.isArray(equipos) && equipos.map(e => (
                    <tr key={e.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-5">
                        <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border ${(e.tipo_equipo || e.tipo) === 'administradores' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20 shadow-lg shadow-rose-500/5' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}>
                          {e.tipo_equipo || e.tipo}
                        </span>
                      </td>
                      <td className="px-8 py-5">
                        <code className="text-[10px] font-mono text-slate-400 bg-black/40 px-2 py-1 rounded-lg border border-white/5">{e.telegram_chat_id || e.chat_id}</code>
                      </td>
                      <td className="px-8 py-5">
                      ...
                      <td className="px-8 py-5 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setEditingEquipo(e); setEquipoForm({ nombre: e.nombre_equipo || e.nombre, tipo: e.tipo_equipo || e.tipo, chat_id: e.telegram_chat_id || e.chat_id, activo: !!e.activo }); setShowEquipoModal(true); }} className="p-2 rounded-lg bg-white/5 text-blue-500 hover:bg-blue-500 hover:text-white transition-all shadow-lg border border-white/5"><Edit3 size={14} /></button>
                          <button onClick={async () => { if(confirm('¿Eliminar equipo?')) { await api.admin.telegram.eliminarEquipo(e.id); fetchData(); } }} className="p-2 rounded-lg bg-white/5 text-rose-500 hover:bg-rose-500 hover:text-white transition-all shadow-lg border border-white/5"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          >
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-xl bg-indigo-500/10 text-indigo-500 border border-indigo-500/20 shadow-inner">
                  <Users size={20} />
                </div>
                <div>
              ...
              <button 
                ...
                className="admin-button-primary !h-12 !px-6 flex items-center gap-2 !bg-indigo-600 hover:!bg-indigo-500 shadow-indigo-600/20"
              >
              ...
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    <th className="px-8 py-5">Telegram ID</th>
                    ...
                    <tr key={i.id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-black/40 flex items-center justify-center font-black text-indigo-500 text-[10px] border border-white/5 shadow-inner">
                      ...
                      <td className="px-8 py-5">
                        <code className="text-[10px] font-mono text-slate-400 bg-black/40 px-2 py-1 rounded-lg border border-white/5">{i.telegram_user_id || i.telegram_id}</code>
                      </td>
                      <td className="px-8 py-5">
                      ...
                      <td className="px-8 py-5 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setEditingIntegrante(i); setIntegranteForm({ telegram_user_id: i.telegram_user_id || i.telegram_id, nombre_visible: i.nombre_visible || i.nombre, equipo_id: i.equipo_id || '', activo: !!i.activo }); setShowIntegranteModal(true); }} className="p-2 rounded-lg bg-white/5 text-indigo-500 hover:bg-indigo-500 hover:text-white transition-all shadow-lg border border-white/5"><Edit3 size={14} /></button>
                          <button onClick={async () => { if(confirm('¿Eliminar integrante?')) { await api.admin.telegram.eliminarIntegrante(i.id); fetchData(); } }} className="p-2 rounded-lg bg-white/5 text-rose-500 hover:bg-rose-500 hover:text-white transition-all shadow-lg border border-white/5"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
