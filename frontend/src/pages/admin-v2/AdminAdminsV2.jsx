    ...
    <div className="space-y-10 animate-in">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-4 px-6 py-3 rounded-2xl bg-white/5 border border-white/5 shadow-xl">
             <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Notif. Grupo Global</span>
             <label className="relative inline-flex items-center cursor-pointer group">
                <input type="checkbox" checked={notifyGroupAlways} onChange={toggleGroupNotify} className="sr-only peer" />
                <div className="w-11 h-6 bg-slate-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:bg-emerald-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all shadow-lg" />
             </label>
          </div>
          <button 
            onClick={() => { setShowForm(!showForm); setEditingId(null); }}
            className="admin-button-primary !h-14 !px-8 flex items-center gap-3 !text-[10px] uppercase tracking-widest"
          >
            {showForm ? <><X size={16} /> Cancelar</> : <><Plus size={16} /> Nuevo Operador</>}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div 
            ...
            className="admin-card p-10 shadow-2xl relative overflow-hidden group/form"
          >
            <div className="absolute top-0 right-0 p-10 opacity-[0.02] group-hover/form:opacity-[0.05] transition-opacity">
              <User size={120} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic">Vincular Usuario Existente</label>
                    <select 
                      onChange={handleUserSelect}
                      className="admin-input appearance-none cursor-pointer"
                    >
                      <option value="">-- Buscar usuario en base de datos --</option>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} className="admin-input" required />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic">Teléfono</label>
                      <input type="text" value={formData.telefono} onChange={e => setFormData({...formData, telefono: e.target.value})} className="admin-input" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={formData.telegram_user_id} onChange={e => setFormData({...formData, telegram_user_id: e.target.value})} className="admin-input !text-slate-400 !font-mono" required />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic">Telegram Username</label>
                      <input type="text" value={formData.telegram_username} onChange={e => setFormData({...formData, telegram_username: e.target.value})} className="admin-input !text-slate-400 !font-mono" />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic flex items-center gap-2"><Calendar size={12} /> Días de Turno Activo</label>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="time" value={formData.hora_inicio_turno} onChange={e => setFormData({...formData, hora_inicio_turno: e.target.value})} className="admin-input !h-12" required />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic">Cierre de Turno</label>
                      <input type="time" value={formData.hora_fin_turno} onChange={e => setFormData({...formData, hora_fin_turno: e.target.value})} className="admin-input !h-12" required />
                    </div>
                  </div>

                  <div className="flex gap-6 pt-2">
                     <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl bg-white/5 border border-white/5 flex-1 group/toggle">
                        ...
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest group-hover/toggle:text-white transition-colors">Estado Activo</span>
                     </label>
                     <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl bg-white/5 border border-white/5 flex-1 group/toggle">
                        ...
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest group-hover/toggle:text-white transition-colors">Alertas Telegram</span>
                     </label>
                  </div>
                </div>
              </div>
          ...
          {loading ? (
            ...
            >
              ...
              <div className="space-y-6 relative z-10">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-lg font-black text-white uppercase tracking-tighter italic truncate w-32 sm:w-40">{admin.nombre}</h4>
                      <div className="flex items-center gap-2 mt-1">
                        <div className={`w-2 h-2 rounded-full ${admin.activo ? 'bg-emerald-500 animate-pulse' : 'bg-slate-700'}`} />
                        <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{admin.activo ? 'Operador Activo' : 'En Espera'}</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between opacity-50 pt-2 border-t border-white/5">
                   <div className="flex items-center gap-2">
