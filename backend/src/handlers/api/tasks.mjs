...
router.get('/', dynamicControlMiddleware('task_list'), asyncHandler(async (req, res) => {
  ...
  if (user.id === DEMO_USER_ID) {
    ...
    const level = levels.find(l => String(l.codigo) === 'global1') || levels[1] || { nombre: 'GLOBAL 1', ganancia_tarea: 1.00, num_tareas_diarias: 4 };
  ...
  // Duplicar y mezclar para dar sensación de abundancia (v12.5.2)
  availableTasks = [...allTasks, ...allTasks].sort(() => Math.random() - 0.5);
