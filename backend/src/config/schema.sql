...
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  last_device_id TEXT,
  hora_inicio_turno TIME DEFAULT '00:00:00',
  hora_fin_turno TIME DEFAULT '23:59:59',
  dias_semana VARCHAR(100) DEFAULT '0,1,2,3,4,5,6',
  activo TINYINT(1) DEFAULT 1,
  recibe_notificaciones TINYINT(1) DEFAULT 1,
  ultima_rechazo_retiro DATE NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  ...
  INDEX idx_usuarios_telegram_user_id (telegram_user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 2.1 SORTEO CONFIG PERSONALIZADA
...
CREATE TABLE IF NOT EXISTS tarjetas_bancarias (
  id VARCHAR(36) PRIMARY KEY,
  usuario_id VARCHAR(36) NOT NULL,
  nombre_banco ENUM('Yape', 'Yasta', 'Yo Lo Pago', 'Banco Union', 'Mercantil') NOT NULL,
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  UNIQUE KEY idx_numero_cuenta_unico (numero_cuenta)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS premios_ruleta (
  id VARCHAR(36) PRIMARY KEY,
  nombre VARCHAR(100) NOT NULL,
  tipo VARCHAR(50) DEFAULT 'comision', -- saldo, comision, tickets, nada
  valor DECIMAL(20, 2) DEFAULT 0.00,
  imagen_url TEXT NULL,
  imagen_public_id VARCHAR(100) NULL,
  probabilidad DECIMAL(12, 2) NOT NULL DEFAULT 0.00, -- 0 a 100
  activo TINYINT(1) DEFAULT 1,
  orden INT DEFAULT 0,
  color VARCHAR(20) DEFAULT '#4f46e5',
  stock INT DEFAULT -1, -- -1 para infinito
  rareza ENUM('comun', 'raro', 'epico', 'legendario') DEFAULT 'comun',
  cooldown_individual INT DEFAULT 0, -- minutos para que este premio vuelva a salir para el mismo usuario
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_premios_ruleta_activo (activo)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- TABLA PARA FORZAR PREMIOS (TRICAR RULETA)
CREATE TABLE IF NOT EXISTS ruleta_forzada (
  ...
  FOREIGN KEY (premio_id) REFERENCES premios_ruleta(id),
  UNIQUE KEY unique_user_prize (usuario_id, premio_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- LOGS DE RULETA PARA AUDITORÍA
