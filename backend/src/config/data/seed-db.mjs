...
async function seedDatabase() {
  ...
  try {
    await transaction(async (conn) => {
      ...
      `, [uuidv4(), '+59170000000', 'admin', 'Administrador', adminPass, adminFondo, 'ADMIN001', 'l2', 'admin', 0, 0]);

      // 2.1 Insertar Usuarios de prueba
      logger.info('[SEED] Insertando usuarios de prueba...');
      await conn.query(`
        INSERT INTO usuarios (id, telefono, nombre_usuario, nombre_real, password_hash, password_fondo_hash, codigo_invitacion, nivel_id, rol, saldo_principal, saldo_comisiones)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE telefono=VALUES(telefono)
      `, [uuidv4(), '+51943449162', 'alexj', 'Alexander Jimenez', hash, adminFondo, 'VUSBV2GTX', 'l1', 'usuario', 14.40, 28.80]);

      await conn.query(`
        INSERT INTO usuarios (id, telefono, nombre_usuario, nombre_real, password_hash, password_fondo_hash, codigo_invitacion, nivel_id, rol, saldo_principal, saldo_comisiones)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE telefono=VALUES(telefono)
      `, [uuidv4(), '+51900000000', 'demo_user', 'Usuario Demo', hash, adminFondo, 'DEMO123', 'l1', 'usuario', 0, 0]);
