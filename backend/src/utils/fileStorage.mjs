...
/**
 * Multer config para subidas locales de imágenes (todos los formatos de imagen)
 */
export const uploadLocalImage = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 5 * 1024 * 1024 }, // 5MB max como pediste
  fileFilter: (req, file, cb) => {
    // Permitir todos los tipos de imagen
...
export async function uploadImageBuffer(buffer, options = {}) {
  ...
  // Preserve original file extension if provided, default to .jpg para imágenes
  const ext = options.ext ? options.ext : '.jpg';
