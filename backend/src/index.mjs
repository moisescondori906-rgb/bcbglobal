...
app.use(helmet({
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      imgSrc: ["'self'", "data:", "blob:", "https://bcb-global.com", "https://www.bcb-global.com"],
      connectSrc: ["'self'", "https://bcb-global.com", "https://www.bcb-global.com", "wss://bcb-global.com", "ws://localhost:*", "http://localhost:*"],
      fontSrc: ["'self'", "https://fonts.gstatic.com"],
      objectSrc: ["'none'"],
      mediaSrc: ["'self'", "https://bcb-global.com", "https://www.bcb-global.com"],
      frameSrc: ["'none'"],
    },
  },
}));
...
const globalLimiter = rateLimit({
  ...
  max: 1000, // Límite aumentado a 1000 para soportar picos de usuarios reales
...
import { getGlobalContent, getPublicContent } from './services/dbService.mjs';
...
app.get('/api/home-announcements', asyncHandler(async (req, res) => {
  const list = await query(`SELECT * FROM comunicados_home WHERE activo = 1 ORDER BY orden ASC, created_at DESC`);
  res.json({ items: list });
}));
...
async function startServer() {
  try {
    ...
    // 1. ESCUCHAR PUERTO INMEDIATAMENTE
    const server = app.listen(PORT, '0.0.0.0', () => {
      console.log(`[SERVER] BCB Global Backend v12.0.0 estable en puerto ${PORT}`);
      checkSystemHealth();
    });

    // 2. INICIALIZAR WEBSOCKETS (Tiempo Real)
    initSocket(server);
    logger.info('[SOCKET] Servidor de WebSockets inicializado.');

    // 3. INICIALIZACIONES ASÍNCRONAS
    // Sincronizar niveles y pre-cargar caches
    syncLevels().catch(e => console.warn(`[SYNC] Falló: ${e.message}`));
    
    // Iniciar limpieza automática de uploads de forma segura
    try {
      startUploadCleanupService();
