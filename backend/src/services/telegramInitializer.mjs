...
export async function initTelegramHandlers() {
  ...
  try {
    const { setupAdminBot, setupRetirosBot, setupSecretariaBot } = await import('./telegramBot.mjs');
    ...
    try {
      const { setupTelegramLogic } = await import('./telegramService.mjs');
      await setupTelegramLogic();
      logger.info('[TELEGRAM] Lógica de eventos cargada.');
    } catch (err) {
      logger.error('[TELEGRAM] Error al cargar lógica de eventos:', {
        message: err.message,
        stack: err.stack
      });
    }
