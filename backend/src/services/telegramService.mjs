      ...
      try {
        ...
        await transaction(async (conn) => {
          ...
          if (action === 'tomar') {
            ...
            // 2. Marcar como tomado en bloqueo (actualiza el responsable actual)
            await conn.query(`
            ...
            // 3. Actualizar estado operativo en la tabla real (compras_nivel o retiros)
            const table = caso.tipo_operacion === 'retiro' ? 'retiros' : 'compras_nivel';
            
            // Verificamos si las columnas existen en la tabla destino
            const [columns] = await conn.query(`SHOW COLUMNS FROM ${table}`);
            const colNames = columns.map(c => c.Field);
            
            let updateQuery = `UPDATE ${table} SET estado_operativo = 'tomado'`;
            const params = [];

            if (colNames.includes('taken_by_admin_id')) {
              updateQuery += `, taken_by_admin_id = ?`;
              params.push(adminId);
            }
            if (colNames.includes('taken_by_admin_name')) {
              updateQuery += `, taken_by_admin_name = ?`;
              params.push(adminName);
            }
            if (colNames.includes('taken_at')) {
              updateQuery += `, taken_at = ?`;
              params.push(boliviaTime.now());
            }
            ...
            await safeTelegramCall(() => bot.answerCallbackQuery(callbackId, { text: `✅ Caso tomado correctamente.` }), 'answerCallbackQuery-tomar-success');
            await updateTelegramMessage(bot, message, 'tomado', displayerName, refId, caso.tipo_operacion);
          }
